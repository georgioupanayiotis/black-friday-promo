...
export class BlackFridayPromo {
    ...
    private getRibbonTransform(position: string): string {
        switch (position) {
    ...
    private setRibbonPosition(element: HTMLElement, position: string): void {
    ...
    createMarquee(config: MarqueeConfig = {}): HTMLElement {
        const {
            text = '🔥 BLACK FRIDAY SALE 🔥 50% OFF 🔥 LIMITED TIME ONLY 🔥',
            position = 'inline',
    ...
    private addMarqueeAnimation(direction: string): void {
        ...
        switch (trigger) {
            ...
            case 'scroll':
                window.addEventListener('scroll', () => {
                    ...
                    if (scrolled >= scrollPercentage) showPopup();
                }, { once: true });
                break;
        }

        overlay.onclick = (e) => {
            if (e.target === overlay) this.hidePopup(overlay, popup);
        };

        return overlay;
    }

    private hidePopup(overlay: HTMLElement, popup: HTMLElement): void {
    ...
    createCountdown(config: CountdownConfig): HTMLElement {
        ...
        const updateCountdown = () => {
            ...
            let timeString = '⏰ ';
            if (showDays) timeString += `${days}d `;
            if (showHours) timeString += `${hours}h `;
            if (showMinutes) timeString += `${minutes}m `;
            if (showSeconds) timeString += `${seconds}s`;
    ...
    createFloatingButton(config: FloatingButtonConfig = {}): HTMLElement {
        ...
        Object.assign(fab.style, {
            ...
            zIndex: zIndex.toString(),
            transition: 'transform 0.3s ease',
            animation: pulse ? 'bfp-pulse 2s infinite' : 'none'
        });
    ...
    private addPulseAnimation(): void {
        if (document.getElementById('bfp-pulse-style')) return;
    ...
    private addToastProgressAnimation(): void {
        if (document.getElementById('bfp-toast-progress-style')) return;
...
export default new BlackFridayPromo();
